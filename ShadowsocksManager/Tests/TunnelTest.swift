...
import Tun2socks
@testable import ShadowsocksManager

final class TunnelTest: XCTestCase {
    
    static let kAppGroup = "shadowsocks.spm.tests"
    
    func testTunnelStoreDoesNotLoadBeforeSetup() throws {
        let tunnelStore = TunnelStore(appGroup: TunnelTest.kAppGroup)
    ...
    func testGetTunnelNetworkSettings() {
        let settings = Tunnel.getTunnelNetworkSettings(tunnelRemoteAddress: "1.2.3.4")
